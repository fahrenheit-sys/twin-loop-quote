...
const STATIONS = {
  wire:     { sku: 'B0010001', title: 'Twin Loop Wire Binding', account: '4-1000' },
...
/**
 * Builds the PostOrder payload for one quantity tier of a quote.
 *
 * Wayne wants every quantity the customer asked for to reach Hexicom as its own
 * order (2026-09-24), so he can convert whichever one the customer takes into a
 * job and leave the rest. Each order carries the full list of tiers in its Notes
 * so the whole quote is in front of him from any one of them.
 */
function buildOrder({ state: rawState, computed, testMode = false, idx = 0, externalId }) {
  ...
  const count = (state.qtys || []).length;
  const notes = clean(
    `Quote ${quoteNo}. Quantity options (ex GST): ${tiers}. ` +
    (count > 1 ? `This order is the ${state.qtys[idx]} option (${idx + 1} of ${count}). ` : '') +
    extrasText(state)
  );
  ...
  return {
    ...
    ExternalOrderID: (testMode ? 'TEST-' : '') + (externalId || quoteNo),
    // CustomerOrderID feeds "Customer Order No" on the job ticket, which is where
    // Wayne types the customer's purchase order number — it has to reach him
    // empty. We were filling it with the quote number, so he had to clear it on
    // every job. The quote number travels in OrderNotes, ExternalOrderID and each
    // item's Notes instead. Wayne, 2026-09-10.
    OrderNotes: count > 1 ? `Estimate ${quoteNo} — quantity ${state.qtys[idx]}` : `Estimate ${quoteNo}`,
...
/**
 * One order per quantity tier. A single-quantity quote keeps the bare quote
 * number as its ExternalOrderID, exactly as before; with several tiers each id
 * carries its quantity ("2026-09-17-P798-500"), which keeps them unique — Hexicom
 * rejects a repeated id — and still finds all of them on a search for the quote.
 * A quantity entered twice is one order, not two.
 */
function buildOrders({ state, computed, testMode = false }) {
  ...
  qtys.forEach((q, idx) => {
    ...
    seen.add(key);
    const externalId = qtys.length > 1 ? `${state.quoteNumber}-${key}` : state.quoteNumber;
    orders.push(buildOrder({ state, computed, testMode, idx, externalId }));
  });
  return orders;
}

// Rows written before 2026-09-24 hold a single payload object; newer rows hold
// one payload per quantity tier.
const asPayloadList = p => (Array.isArray(p) ? p : p ? [p] : []);

/**
 * Posts every order in the list, skipping any that an earlier attempt already
 * got through (`previous` is the per-order result list stored on the quote).
 * Re-posting one that exists would only earn Hexicom's generic 500, which is
 * what used to flip a sent quote to "Failed" on a retry.
 */
async function postOrders(payloads, previous = []) {
  const done = new Map((previous || []).filter(r => r && r.ok).map(r => [r.id, r]));
  const results = [];
  for (const payload of asPayloadList(payloads)) {
    const id  = payload.ExternalOrderID;
    const qty = payload.OrderItems && payload.OrderItems[0] ? payload.OrderItems[0].Quantity : null;
    if (done.has(id)) { results.push(done.get(id)); continue; }
    let r;
    try { r = await postOrder(payload); } catch (e) { r = { ok: false, error: e.message }; }
    results.push({
      id, qty, ok: !!r.ok,
      orderNo: r.orderNo || null,
      itemNos: r.itemNos || null,
      error:   r.ok ? null : String(r.error || '').slice(0, 500),
      maybeDuplicate: !!r.maybeDuplicate,
    });
  }
  return results;
}

// The columns the quote row keeps about its Hexicom send. `hexicom_item_nos`
// carries the per-order results as JSON so a retry knows which orders to skip;
// `hexicom_order_no` is the readable list the admin shows.
function hexicomColumns(results) {
  const sent = results.filter(r => r.ok);
  const status = !results.length ? 'failed'
               : sent.length === results.length ? 'sent'
               : sent.length ? 'partial' : 'failed';
  const multi = results.length > 1;
  const orderNo = sent.map(r => (r.orderNo || r.id) + (multi && r.qty ? ` (qty ${r.qty})` : '')).join(', ') || null;
  const errors = results.filter(r => !r.ok).map(r => `${r.id}: ${r.error || 'failed'}`).join('\n');
  return {
    hexicom_status:   status,
    hexicom_order_no: orderNo,
    hexicom_item_nos: JSON.stringify(results),
    hexicom_sent_at:  new Date().toISOString(),
    hexicom_error:    errors ? errors.slice(0, 1000) : null,
  };
}
...
module.exports = { buildOrder, buildOrders, postOrder, postOrders, hexicomColumns, asPayloadList, STATIONS, ENDPOINT };
