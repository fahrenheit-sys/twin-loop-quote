...
// Wire, spiral and comb are priced by the length of the bound edge, so the size
// step asks the customer to drop a size when they bind on the short edge (an A3
// book bound short-edge is quoted as A4, because A4's long edge and A3's short
// edge are both 297mm). That makes state.leafSize the COSTING size, not the book
// the operator will hold. Wayne needs the ticket to name the real book and the
// millimetre span the wire has to cover, so we reverse the rule here.
//
// Either way the bound edge is the LONG edge of the costing size: on a long-edge
// bind that is the book's own long edge; on a short-edge bind the step-down was
// made precisely so those two lengths match.
const SIZE_MM = {
  A2: [420, 594], A3: [297, 420], A4: [210, 297], A5: [148, 210], A6: [105, 148],
  B4: [250, 353], B5: [176, 250],
};
// The size a short-edge book actually is, given the size it was costed at.
const SIZE_UP = { A6: 'A5', A5: 'A4', A4: 'A3', A3: 'A2', B5: 'B4' };

// "A3, bound on the short (297mm) edge" — the finished size and the wire length
// in one phrase, so neither reading of the ticket can put the wrong wire on.
// Falls back to the bare costing size if we don't hold dimensions for it.
function sizeAndEdge(state) {
  const costing = state.leafSize || '';
  if (!isPunched(state) || !state.bindEdge) return costing;

  const short = /short/i.test(state.bindEdge);
  const book  = short ? (SIZE_UP[costing] || '') : costing;
  const mm    = (SIZE_MM[costing] || [])[1] || 0;
  if (!book || !mm) return costing + (state.bindEdge ? `, bound on ${state.bindEdge.toLowerCase()}` : '');

  return `${book}, bound on the ${short ? 'short' : 'long'} (${mm}mm) edge`;
}

// The Size box on the ticket. Same correction as the description, kept short.
function sizeField(state) {
  ...
  const book  = short ? (SIZE_UP[costing] || '') : costing;
  const mm    = (SIZE_MM[costing] || [])[1] || 0;
  if (!book || !mm) return costing;

  return `${book} (bound edge ${mm}mm)`;
}
